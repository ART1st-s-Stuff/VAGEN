...
class JointTrainingConfigWiringTest(unittest.TestCase):
    ...
    def _enable_id169_gate(self, config, *, phase="update_1"):
        ...
        with open_dict(config):
            ...
            config.data = {
                "train_batch_size": 8,
                "train_files": "/repo/train_navigation_joint_id169.yaml",
            }
        ...
        with open_dict(config.trainer):
            ...
            config.trainer.experiment_name = (
                "169_smoke_vagenlite_jointupdate_dp8_tp8_gate"
            )
            config.trainer.logger = ["console", "wandb"]
            config.trainer.val_before_train = False
            config.trainer.test_freq = -1
            config.trainer.default_local_dir = "/outputs/id169/checkpoints"
...
"""
    ...
    def test_id169_config_constructs_disabled_hf_upload_manager(self) -> None:
        from omegaconf import OmegaConf
        from vagen.utils.upload_hugging_face import HFUploadManager

        config_path = Path(__file__).parents[1] / "vagen/configs/joint_id169_gate.yaml"
        source_config = OmegaConf.load(config_path)
        runtime_config = OmegaConf.create(
            {
                "huggingface_hub": source_config.huggingface_hub,
                "trainer": {
                    "default_local_dir": "/tmp/id169-checkpoints",
                    "project_name": "vagen",
                    "experiment_name": "id169-config-test",
                },
            }
        )
    ...
    def test_allows_only_human_approved_id169_integration_gate(self) -> None:
        from vagen.main_ppo import _configure_joint_actor_extension

        config = self._enable_id169_gate(self._config())
        training = _configure_joint_actor_extension(config)
        self.assertEqual(training.run_seed, 42001)
        self.assertEqual(config.trainer.save_freq, 1)
        config = self._enable_id169_gate(self._config())
        config.joint_integration_gate.experiment_id = 168
        with self.assertRaisesRegex(ValueError, "169"):
