...
class JointTrainingConfigWiringTest(unittest.TestCase):
    ...
    def _enable_id167_gate(self, config, *, phase="update_1"):
        ...
        with open_dict(config):
            config.joint_integration_gate = {
                "enabled": True,
                "implementation": "id167_dp8_resume_smoke_v1",
                "experiment_id": 167,
                "phase": phase,
            }
            ...
            config.data = {
                "train_batch_size": 8,
                "train_files": "/repo/train_navigation_joint_id167.yaml",
            }
        ...
        with open_dict(config.trainer):
            ...
            config.trainer.experiment_name = (
                "167_smoke_vagenlite_jointupdate_dp8_tp8_gate"
            )
            config.trainer.logger = ["console", "wandb"]
            config.trainer.val_before_train = False
            config.trainer.test_freq = -1
            config.trainer.default_local_dir = "/outputs/id167/checkpoints"
...
"""
    ...
    def test_id167_config_constructs_disabled_hf_upload_manager(self) -> None:
        from omegaconf import OmegaConf
        from vagen.utils.upload_hugging_face import HFUploadManager

        config_path = Path(__file__).parents[1] / "vagen/configs/joint_id167_gate.yaml"
        source_config = OmegaConf.load(config_path)
        runtime_config = OmegaConf.create(
            {
                "huggingface_hub": source_config.huggingface_hub,
                "trainer": {
                    "default_local_dir": "/tmp/id167-checkpoints",
                    "project_name": "vagen",
                    "experiment_name": "id167-config-test",
                },
            }
        )
    ...
    def test_allows_only_human_approved_id167_integration_gate(self) -> None:
        from vagen.main_ppo import _configure_joint_actor_extension

        config = self._enable_id167_gate(self._config())
        training = _configure_joint_actor_extension(config)
        ...
        with self.assertRaisesRegex(ValueError, "167"):
