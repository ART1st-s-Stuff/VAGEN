...
class GymImageEnvClient(GymImageEnv):
    ...
    def __init__(self, env_config: Dict[str, Any]):
        ...
        # Spread first connection attempts across URL pools. Session-bound calls
        # remain sticky to the URL that returned the session_id.
        self._current_url_index: int = random.randrange(len(self.base_urls)) if self.base_urls else 0
    ...
    async def _call(
        ...
        for attempt in range(self.retries + 1):
            # Session-bound calls must stay on the server that created the
            # session. Failing over /call to a different env server produces
            # false 404s because session ids are local to each server process.
            url_index = self._current_url_index
            base_url = self.base_urls[url_index]
            url = f"{base_url}/call"
            ...
            try:
                ...
                # Keep current URL sticky for this session.
