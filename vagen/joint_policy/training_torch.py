...
def k4_guided_action_ppo_terms(
    *,
    current_prior_logits: Any,
    frozen_planner_root_mean_values: Any,
    guided_action_ids: Any,
    behavior_guided_log_probs: Any,
    advantages: Any,
    valid_mask: Any,
    ...
    """K4 Scheme-B PPO; gradients enter only current Qwen prior logits."""

    import torch

    if not isinstance(policy_config, K4MCTSGuidedPolicyConfig):
        raise TypeError("K4 guided PPO requires K4MCTSGuidedPolicyConfig")
    if not isinstance(clip_ratio, (float, int)) or isinstance(clip_ratio, bool):
        raise ValueError("K4 guided PPO clip_ratio must be a finite real")
    clip = float(clip_ratio)
    if not torch.isfinite(torch.tensor(clip)) or clip <= 0.0 or clip >= 1.0:
        raise ValueError("K4 guided PPO clip_ratio must be in (0, 1)")
    if current_prior_logits.ndim != 2:
        raise ValueError("K4 guided PPO current prior logits must have shape (B, A)")
    batch_size, action_count = current_prior_logits.shape
    if (
        action_count < 1
        or frozen_planner_root_mean_values.shape != current_prior_logits.shape
    ):
        raise ValueError("K4 guided PPO planner scores must align with prior logits")
    for field, tensor in (
        ("guided_action_ids", guided_action_ids),
        ("behavior_guided_log_probs", behavior_guided_log_probs),
        ("advantages", advantages),
        ...
        if tuple(tensor.shape) != (batch_size,):
            raise ValueError(f"K4 guided PPO {field} must have shape (B,)")
    if guided_action_ids.dtype != torch.long:
        raise ValueError("K4 guided PPO action IDs must be torch.long")
    if torch.any(guided_action_ids < 0) or torch.any(
        guided_action_ids >= action_count
    ):
        raise ValueError("K4 guided PPO action ID is outside the action table")
    if not torch.isfinite(behavior_guided_log_probs).all():
        raise ValueError("K4 guided PPO behavior log-probs must be finite")
    if not torch.isfinite(advantages).all():
        raise ValueError("K4 guided PPO advantages must be finite")
    planner = frozen_planner_root_mean_values.detach().to(
        device=current_prior_logits.device,
        dtype=current_prior_logits.dtype,
    )
    if not torch.isfinite(planner).all():
        raise ValueError("K4 guided PPO planner scores must be finite")
    scaled_prior = current_prior_logits / policy_config.prior_temperature
    guided_logits = (
        policy_config.alpha * scaled_prior
        + policy_config.beta * planner
    )
    guided_log_probs = torch.log_softmax(guided_logits, dim=-1)
    selected = guided_log_probs.gather(
        -1,
        guided_action_ids.unsqueeze(-1),
    ).squeeze(-1)
    behavior = behavior_guided_log_probs.detach().to(
        device=selected.device,
        dtype=selected.dtype,
    ...
    mask = valid_mask.to(device=selected.device, dtype=torch.bool)
    ratios = torch.exp(selected - behavior)
    clipped_ratios = torch.clamp(ratios, 1.0 - clip, 1.0 + clip)
    surrogate = torch.minimum(ratios * advantage, clipped_ratios * advantage)
    policy_loss_sum = -(surrogate * mask).sum()
    ...
    entropy_sum = (entropy * mask).sum()
    if not torch.isfinite(policy_loss_sum) or not torch.isfinite(entropy_sum):
        raise ValueError("K4 guided PPO loss terms must be finite")
    return GuidedActionPPOTerms(
        policy_loss_sum=policy_loss_sum,
        entropy_sum=entropy_sum,
        valid_count=mask.sum(),
