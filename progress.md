## 项目进度
目前仍处于原型阶段，代码中已有 world model / latent planner 的初步接线，但整体训练闭环和实验闭环都还没有真正跑通。
...
- [ ] 增加 action parser / validator：
  - 能从 VLM 文本输出稳定解析出 action id。
  - 出错时可回退、可统计解析失败率。
- [ ] 在 rollout / dataset 中补齐 predictor 监督字段：
...
### 0.1 Action 映射单点定义
- [ ] 建立唯一的 action schema，至少包含：
  - `action_name`
  - `action_token`
  - `action_id`
  - `env_action`
- [ ] 目标是把当前多处重复定义收敛到一个共享位置，避免后续映射漂移。
- [ ] 当前可重点检查/收敛的文件：
...
- [ ] 明确兼容策略：
  ...
  - 若格式错误，记录错误类型并拒绝静默吞错。
- [ ] 当前已有 fallback，但它更像临时兜底，后续应改为可观测、可统计的显式策略。
- [ ] 验收标准：
  - 能统计 `planner_triggered`、`planner_fallback_used`、`planner_parse_failed`。
  - 可以区分“模型自己选动作”和“模型只触发 planner”两种轨迹。

### 0.3 明确 predictor 监督字段契约
- [ ] 写清楚训练 batch 中每个字段的语义、shape、dtype、来源：
...
- [ ] 推荐把“真正执行到环境里的动作”作为监督目标，而不是模型原始文本里声明的动作。
...
## 当前阶段的核心 TODO
- [ ] 收敛 action schema，消除多处重复定义。
- [ ] 固定 `latent_plan` 的 trigger 协议与 fallback 策略。
- [ ] 打通 `action_labels / step_rewards / next_latent` 数据流。
