...
: "${NUM_ACTIONS:=8}"
: "${WORLD_STATE_DIM:=256}"
: "${PREDICTOR_MODE:=world_state_with_decoder}"   # world_state_with_decoder|direct_latent_mlp|lewm_latent_dynamics|world_state_mlp
: "${PREDICTOR_HORIZON:=2}"
...
PYTHONUNBUFFERED=1 python -m vagen.main_ppo \
  ...
  actor_rollout_ref.actor.predictor_mode="$PREDICTOR_MODE" \
  actor_rollout_ref.actor.predictor_multi_step_horizon="$PREDICTOR_HORIZON" \
