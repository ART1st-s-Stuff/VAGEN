...
if [ "$GPU_COUNT" -lt 4 ]; then
  echo "preflight together full 4GPU: expected at least 4 GPUs, saw $GPU_COUNT" >&2
  nvidia-smi >&2 || true
  exit 1
fi
...
export SERVER_PORT=${SERVER_PORT:-$((5600 + RANDOM % 300))}
...
export EXPERIMENT_NAME=${EXPERIMENT_NAME:-zz_delete_preflight_full_4gpu_together_gptoss20b_$(date -u +%Y%m%dT%H%M%SZ)}
...
export VAGEN_AI2THOR_HOME=${VAGEN_AI2THOR_HOME:-/tmp/${USER:-hligb}/vagen-navigation/${SLURM_JOB_ID:-manual}/ai2thor-preflight4}
...
echo "preflight together full 4GPU: provider=$VAGEN_JUDGE_PROVIDER model=$VAGEN_JUDGE_MODEL port=$SERVER_PORT"
source scripts/superpod/start_local_server.sh
bash scripts/examples/vagen_full/navigation/run.sh
cleanup_vagen_server || true
delete_preflight_wandb
echo "preflight together full 4GPU: completed"
