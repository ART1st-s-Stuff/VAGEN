...
export WANDB_RUN_GROUP=${WANDB_RUN_GROUP:-navigation_vagen1_external2_debug5_20260722}
export WANDB_TAGS=${WANDB_TAGS:-vagen,navigation,vagen1,step1,external-ai2thor,multi-server,debug5,normal,4gpu}
...
submit_log="$VAGEN_LOG_ROOT/vagen1-external-multi-env-debug-submit-${RUN_STAMP}.log"
...
for variant in "${variants[@]}"; do
  ...
  (
    ...
    echo "$variant env_jobs=$(IFS=,; echo "${server_jobs[*]}") train_job=$train_job ready_files=$server_ready_files_csv train_batch=$TRAIN_BATCH_SIZE rollout_mini=$ROLLOUT_MINI_BATCH_SIZE server_workers=$SERVER_NAVIGATION_MAX_WORKERS env_servers=$ENV_SERVERS_PER_VARIANT"
  )
done | tee "$submit_log"
