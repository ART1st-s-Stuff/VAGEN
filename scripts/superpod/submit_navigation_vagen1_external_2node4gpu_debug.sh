...
export WANDB_TAGS=${WANDB_TAGS:-vagen,navigation,vagen1,step1,external-ai2thor,multi-server,debug5,normal,train2x4}
...
export ENV_SERVERS_PER_VARIANT=${ENV_SERVERS_PER_VARIANT:-2}
...
submit_log="$VAGEN_LOG_ROOT/vagen1-external-2node4gpu-debug-submit-${RUN_STAMP}.log"
...
for variant in "${variants[@]}"; do
  ...
  (
    ...
    for server_idx in $(seq 1 "$ENV_SERVERS_PER_VARIANT"); do
      server_session="navigation_vagen1_${variant}_${RUN_STAMP}_${variant_idx}_server${server_idx}"
      ...
      server_jobs+=("$server_job")
    done

    server_jobs_dependency=$(IFS=:; echo "${server_jobs[*]}")
    server_ready_files_csv=$(IFS=,; echo "${ready_files[*]}")

    train_job=$(sbatch --parsable --time="$TRAIN_TIME" --dependency=after:${server_jobs_dependency} \
      --export=ALL,VAGEN_REPO="$VAGEN_REPO",VAGEN_ARTIFACT_ROOT="$VAGEN_ARTIFACT_ROOT",SERVER_SESSION_ID="navigation_vagen1_${variant}_${RUN_STAMP}_${variant_idx}",SERVER_READY_FILE="${ready_files[0]}",SERVER_READY_FILES="$server_ready_files_csv",SERVER_JOB_IDS="$server_jobs_dependency",VAGEN1_VARIANT="$variant",RUN_STAMP="$RUN_STAMP",TOTAL_TRAINING_STEPS="$TOTAL_TRAINING_STEPS",SAVE_FREQ="$SAVE_FREQ",TEST_FREQ="$TEST_FREQ",VAL_BEFORE_TRAIN="$VAL_BEFORE_TRAIN",FINAL_VAL_AFTER_TRAIN="$FINAL_VAL_AFTER_TRAIN",FORCE_GEN_DATA="$FORCE_GEN_DATA",REMOVE_PREVIOUS_CKPT_IN_SAVE="$REMOVE_PREVIOUS_CKPT_IN_SAVE",ENV_CONFIG_PATH="$ENV_CONFIG_PATH",VAL_BATCH_SIZE="$VAL_BATCH_SIZE",SERVER_WAIT_SECONDS="$SERVER_WAIT_SECONDS",TRAIN_NNODES="$TRAIN_NNODES",N_GPUS_PER_NODE="$N_GPUS_PER_NODE",EXPECTED_RAY_GPUS="$EXPECTED_RAY_GPUS" \
      scripts/superpod/run_navigation_vagen1_2node4gpu_external_server.sbatch)
    if [[ ! "$train_job" =~ ^[0-9]+$ ]]; then
      echo "sbatch did not return a numeric 2node4gpu training job id: $train_job" >&2
      exit 5
    fi

    echo "$variant env_jobs=$(IFS=,; echo "${server_jobs[*]}") train_job=$train_job ready_files=$server_ready_files_csv train_nodes=$TRAIN_NNODES train_gpus_per_node=$N_GPUS_PER_NODE train_batch=$TRAIN_BATCH_SIZE rollout_mini=$ROLLOUT_MINI_BATCH_SIZE server_workers=$SERVER_NAVIGATION_MAX_WORKERS env_servers=$ENV_SERVERS_PER_VARIANT"
  )
done | tee "$submit_log"
